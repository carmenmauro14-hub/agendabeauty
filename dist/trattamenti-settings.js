...
function mostraIcone(container, onSelect, iconaPredefinita = "") {
  ...
  iconeDisponibili.forEach(nome => {
    ...
    if (img.src === iconaPredefinita) img.classList.add("selezionata");
...
btnSalvaModifiche.addEventListener("click", async () => {
  ...
  if (!nome || isNaN(prezzo) || prezzo < 0 || !icona) return alert("Compila tutti i campi e seleziona un’icona.");
  try {
  ...
  snapshot.forEach(docSnap => {
    ...
    li.innerHTML = `
      <div class="trattamento-info">
        ...
        <span class="prezzo-trattamento">€${prezzo.toFixed(2)}</span>
      </div>
...
listaTrattamenti.addEventListener("click", async e => {
  ...
  if (e.target.classList.contains("btn-edit")) {
    ...
    const prezzo = parseFloat(li.querySelector(".prezzo-trattamento").textContent.replace("€",""));
    const icona = li.querySelector("img").src;
