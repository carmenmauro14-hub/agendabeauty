...
async function caricaStatisticheCliente(clienteId) {
  ...
  // aggiorna lifetime (solo numero, il simbolo € è nell'HTML)
  lifetimeSpentEl.textContent = formatNumIT(lifetimeSpent);
...
function renderStatsForYear(year) {
  ...
  // Aggiorna totali (solo numeri; l'€ è nell'HTML)
  yearTreatmentsCountEl.textContent = String(totalTreatments);
  yearSpentEl.textContent = formatNumIT(totalSpentYear);

  // Lista trattamenti formattata: "2 Microblading   Tot. € 700,00"
  const items = Object.entries(freq)
    ...
    .map(([nome, v]) => `
      <li class="riga-trattamento">
        ...
        <span class="totale">Tot. € ${formatNumIT(v.spend)}</span>
      </li>
    `)
    .join("");
